'''
flooding_showcase.py

Module: Flooding Protocol – Showcase Simulation
-----------------------------------------------

Simulates basic Flooding protocol on a random network topology using NetworkX.
...
Responsible for:
    ...
    - Propagating messages until TTL expires
...
Related:
    - flooding_stats.py: Statistical multi-seed evaluation
    - epidemic_showcase.py / epidemic_stats.py: Gossip-style comparison
...
# ===============================
# protocol_eval/showcase/flooding_showcase.py
# or stats/flooding_stat.py
# ===============================
...
from protocols import flooding   # <- change per protocol
...
def main():
    ...
    for run in range(RUNS):
        ...
        history, deliveries = simulator.run(
            G, protocol=flooding.protocol, seeds=seeds, ticks=SIM_TICKS
        )
        ...
        if MODE == "showcase" and run == 0:
            visualize.animate(G, positions, history, title=f"{PROTOCOL_NAME} ({SEED_COUNT} seed)")
